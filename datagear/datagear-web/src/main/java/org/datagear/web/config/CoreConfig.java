...
public class CoreConfig implements InitializingBean
{
	...
	public SqlSessionFactory sqlSessionFactory()
	{
		try
		{
			...
			List<String> pathList = Arrays.asList(mybatisXmlPath.split(","));
			for (String path : pathList) {
				Resource[] mapperResources = pathResolver.getResources(path);
				for (Resource tmp : mapperResources) {
					set.add(tmp);
				}
			}
			Resource[] result = (Resource[]) set.toArray(new Resource[0]);
			SqlSessionFactoryBean bean = new SqlSessionFactoryBean();
			bean.setDataSource(this.dataSourceConfig.dataSource());
			bean.setMapperLocations(result);
