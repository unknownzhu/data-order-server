...
layui.define(['laytpl', 'form', 'util'], function (exports) {
    ...
    TreeTable.prototype.checkParentCB = function ($tr) {
        ...
        //
        var data = d[options.tree.childName];
        if(data.length!==1){
    ...
    /** 获取父节点数据*/
    TreeTable.prototype.getParentData = function($tr){
        var that=this;
        if(that.getDataByTr($tr).parentid==='0'){
            return null;
        }else {
            var tr_prevAll=$tr.prevAll('tr');
            for(var i in tr_prevAll){
                var index = $tr.data('index').split('-',1).toString();
                var indent = parseInt($tr.data('indent'));
                var tempt=tr_prevAll[i];
                var current=tr_prevAll[i].dataset;
                var current_index=current.index.split('-',1).toString();
                var current_indent=parseInt(current.indent);
                if((current_indent===(indent-1)) && (current_index===index)){
                    return that.getDataByTr($(tempt));
                }
            }
        }
        // var data=[];
